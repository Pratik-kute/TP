-- ============================================================
-- Asset Tracking & Maintenance System - Complete Database Schema v2
-- Multi-Organization Support + Smart Asset Tags + Invoice Storage
...
INSERT INTO organizations (name, short_name) VALUES ('1XL Infra', '1XL');
...
-- Assets (category is free-text, no CHECK constraint; invoice_url for file storage)
...
-- Documents (with file_url for actual file storage)
...
-- ============ SEED ADMIN USER ============
-- Admin user linked to the default "1XL Infra" organization
INSERT INTO users (name, email, password, role, phone, is_active, organization_id)
VALUES (
  'Admin',
  'admin',
  'Asset@1xl.2026',
  ...
  '',
  true,
  (SELECT id FROM organizations WHERE short_name = '1XL' LIMIT 1)
);
...
-- Allow all operations (internal tool using anon key with RLS bypass)
...
-- ============ INDEXES ============
-- Organization ID indexes (for multi-org data isolation)
